<template>
  <div class="bg-bg1 border border-border-2 rounded-lg">
    <div class="px-6 py-4 border-b border-border-2">
      <h2 class="text-lg font-semibold">{{ $t('account.appearance.accent.title') }}</h2>
      <p class="text-sm text-secondary mt-1">{{ $t('account.appearance.accent.description') }}</p>
    </div>
    <div class="px-6 py-6">
      <div class="space-y-6">
        <!-- Color Picker Grid -->
          ...
          >
            ...
            <button
              ...
              :class="[
                ...
                currentAccent === colorOption.name ? 'ring-4 ring-opacity-70 scale-105 shadow-lg' : 'shadow-md hover:shadow-lg'
              ]"
        ...
        <div class="border border-border-2 rounded-lg p-4 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
              <div 
                class="w-8 h-8 rounded-full shadow-md"
                :class="getCurrentAccentBgClass()"
              ></div>
              <div>
