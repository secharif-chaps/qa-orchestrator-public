      ...
      <Pagination.List v-slot="{ items }" class="flex items-center gap-2">
        ...
        <template v-for="(item, index) in items" :key="index">
          ...
          <Pagination.ListItem
            ...
            :class="{ 'bg-primary/10 border-primary text-primary': item.value === companyStore.currentPage, 'bg-bg1 text-secondary hover:text-primary': item.value !== companyStore.currentPage }"
            :value="item.value" 
          >
            {{ item.value }}
          </Pagination.ListItem>
