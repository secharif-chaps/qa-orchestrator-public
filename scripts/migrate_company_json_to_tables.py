...
def migrate_profile(session, company_id: int, profile: dict) -> bool:
    ...
    try:
        ...
        """), {
            ...
            'insights': insights, 'insights_source': insights_source,
            'group_name': group_name, 'group_name_source': group_name_source,
            'business_line': business_line, 'business_line_source': business_line_source,
            'catchphrase': catchphrase, 'catchphrase_source': catchphrase_source,
            'establishment_year': establishment_year, 'establishment_year_source': establishment_year_source,
            'employee_count': employee_count, 'employee_count_source': employee_count_source,
            'revenue': revenue, 'revenue_source': revenue_source,
            'ceo': ceo, 'ceo_source': ceo_source,
            'hq': hq, 'hq_source': hq_source,
        })
        return True
...
def migrate_digital(session, company_id: int, digital: dict) -> bool:
    ...
    try:
        ...
        """), {
            'company_id': company_id,
            'insights': insights,
            'overall_strategy': overall_strategy, 'overall_strategy_source': overall_strategy_source,
            'digital_transformation': digital_transformation, 'digital_transformation_source': digital_transformation_source,
            'ecommerce_capabilities': ecommerce, 'ecommerce_capabilities_source': ecommerce_source,
            'mobile_strategy': mobile, 'mobile_strategy_source': mobile_source,
            'digital_marketing_approach': marketing, 'digital_marketing_approach_source': marketing_source,
            'loyalty_program': loyalty, 'loyalty_program_source': loyalty_source,
        })
    ...
    try:
        ...
        for event in events:
            if isinstance(event, dict):
                ...
                """), {
                    'company_id': company_id,
                    'date': date, 'date_source': date_source or event_source,
                    'title': title, 'title_source': title_source or event_source,
                    'description': description, 'desc_source': desc_source or event_source,
                    'category': category, 'cat_source': cat_source or event_source,
                    'location': location, 'loc_source': loc_source or event_source,
                    'impact': impact, 'impact_source': impact_source or event_source,
                })
...
def migrate_products(session, company_id: int, products: dict) -> bool:
    ...
    try:
        ...
        """), {
            'company_id': company_id,
            'insights': insights,
            'customer_type': customer_type, 'customer_type_source': customer_type_source,
            'marketing_positioning': marketing_positioning, 'mp_source': mp_source,
        })

        # Migrate product items (range, partner brands, private labels)
        for item_type, key in [('range', 'range'), ('partner_brand', 'partnerBrands'), ('private_label', 'privateLabels')]:
...
def migrate_jobs(session, company_id: int, jobs: dict) -> bool:
    ...
    try:
        ...
        """), {
            'company_id': company_id,
            'total_openings': total_openings, 'total_openings_source': total_openings_source,
            'top_departments': top_departments, 'top_departments_source': top_departments_source,
            'hiring_focus': hiring_focus, 'hiring_focus_source': hiring_focus_source,
            'growth_indicators': growth_indicators, 'growth_indicators_source': growth_indicators_source,
        })
        ...
        for offer in offers:
            if isinstance(offer, dict):
                ...
                """), {
                    'company_id': company_id,
                    'title': title, 'title_source': title_source or offer_source,
                    'location': location, 'location_source': location_source or offer_source,
                    'department': department, 'dept_source': dept_source or offer_source,
                    'description': description, 'desc_source': desc_source or offer_source,
                    'requirements': requirements, 'req_source': req_source or offer_source,
                    'posted_date': posted_date, 'posted_source': posted_source or offer_source,
                })
