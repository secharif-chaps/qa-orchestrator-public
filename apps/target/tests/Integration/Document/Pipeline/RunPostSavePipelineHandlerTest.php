...
namespace App\Tests\Integration\Document\Pipeline;
...
class RunPostSavePipelineHandlerTest extends AbstractApiTestCase
{
    use Factories;
    ...
    public function testQualityReportIsPersistedAfterProcessing(): void
    {
        ...
        $this->transport('quality_processing')
            ->queue()
            ->assertContains(RunPostSavePipelineAction::class);
    ...
    public function testPostSavePipelineCompletedEventIsDispatched(): void
    {
        $dispatchedEvent = null;
        self::getContainer()
            ->get('event_dispatcher')
            ->addListener(
                PostSavePipelineCompletedEvent::class,
                function (PostSavePipelineCompletedEvent $event) use (&$dispatchedEvent): void {
        ...
        $this->assertNotNull($dispatchedEvent, 'PostSavePipelineCompletedEvent should have been dispatched');
        $this->assertSame($documentId, $dispatchedEvent->documentId);
        $this->assertNotEmpty($dispatchedEvent->reportId);
    }

    /**
     * Dispatches a RunPostSavePipelineAction message without processing the transport.
    ...
    private function dispatchDocument(string $url): array
    {
        ...
        $this->bus->dispatch(new RunPostSavePipelineAction(documentId: $documentId));
    ...
    /**
     * Dispatches and immediately processes a RunPostSavePipelineAction message.
     *
     * @return array{documentId: string, watchFileId: string}
     */
