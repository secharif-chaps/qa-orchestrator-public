<template>
  <div class="space-y-6">
    ...
    <div v-if="isLoading">
      ...
      >
        <Icon icon="fa-chart-bar" class="text-sage-700 animate-bounce text-9xl" />
      </div>
    </div>
    <div
      v-show="!isLoading"
...
const displayItems = computed(() => {
  ...
  if (chartBoxWidth.value === 0) {
    ...
    const estimatedWidth = typeof window !== 'undefined' ? window.innerWidth - 200 : 800
    const availableWidth = estimatedWidth - PADDING_BOX
    const calculatedItems = Math.floor(availableWidth / MIN_BAR_WIDTH)
    return Math.max(1, Math.min(calculatedItems, eventsGraph.value.length || 14))
  }
...
watchEffect(() => {
  if (!isLoadingValue.value && eventsGraph.value.length > 0 && chartRef.value && !chartVal) {
...
const datasetData = computed(() => eventsGraph.value.map((event) => event.documentsCount))
const datasetBackgroundColor = computed(() =>
  eventsGraph.value.map((event) =>
    event.hasEvents ? draw('diagonal-right-left', almondColor500.value) : almondColor400.value,
  ),
)
...
const updateChart = () => {
  if (!chartVal || eventsGraph.value.length === 0 || !chartVal.data.datasets[0]) {
    return
  }
  ...
  if (dataLength.value > 0 && scales?.x) {
    const itemsToShow = displayItems.value
    const currentMax = scales.x.max !== undefined ? Number(scales.x.max) : dataLength.value
