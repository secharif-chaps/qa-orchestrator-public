    ...
    <div v-if="filteredItems && filteredItems.length > 0">
      ...
      >
        <FolderItemDisplay
          ...
          mode="grid"
          @view-item="(id) => router.push(`/folders/${route.params.folderId}/companies/${id}`)"
      ...
      <!-- Table View -->
      <div v-else class="border-primary-lighter-stroke overflow-hidden rounded-sm border bg-white">
        <!-- Table Header -->
...
const { canMoveItems } = useFolderPermissions(folderRef)
