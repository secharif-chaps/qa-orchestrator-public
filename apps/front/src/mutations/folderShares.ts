...
export const useCreateFolderShare = defineMutation(() => {
  ...
  const { mutate, mutateAsync, ...mutation } = useMutation({
    ...
    onSuccess: (_data, { folderId }) => {
      ...
      toast.success(t('common.folder.share.added'))
    },

    onError: () => {
      toast.error(t('common.folder.share.addError'))
    },
  })
...
export const useUpdateFolderShare = defineMutation(() => {
  ...
  const { mutate, mutateAsync, ...mutation } = useMutation({
    ...
    onError: (_error, _variables, context) => {
      ...
      toast.error(t('common.folder.share.updateError'))
    },

    onSuccess: () => {
      toast.success(t('common.folder.share.updated'))
    },
...
export const useDeleteFolderShare = defineMutation(() => {
  ...
  const { mutate, mutateAsync, ...mutation } = useMutation({
    ...
    onError: (_error, _variables, context) => {
      ...
      toast.error(t('common.folder.share.removeError'))
    },

    onSuccess: () => {
      toast.success(t('common.folder.share.removed'))
    },
