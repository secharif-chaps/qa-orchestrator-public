...
async def get_global_cost_analysis(
    ...
    global_stats = (
        ...
        .filter(Task.status == TaskStatus.SUCCEEDED, Task.created_at >= start_datetime, Task.created_at <= end_datetime)
        .first()
    )
    ...
    organization_count = (
        db.query(func.count(func.distinct(Company.organization_id)))
        .join(Task, Company.id == Task.company_id)
        .filter(Task.status == TaskStatus.SUCCEEDED, Task.created_at >= start_datetime, Task.created_at <= end_datetime)
        .scalar()
    )
...
async def get_cost_by_organization(
    ...
    query = (
        ...
        .order_by(func.sum(Task.total_cost).desc())
    )
