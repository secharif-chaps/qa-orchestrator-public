...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            logger.warning(
                ...
                extra={"method": request.method, "path": request.url.path, "error": e.detail},
            )
            return JSONResponse(status_code=e.status_code, content={"detail": e.detail, "type": "validation_error"})
            ...
            logger.error(
                ...
                exc_info=True,
                extra={"method": request.method, "path": request.url.path, "error_type": type(e).__name__},
            )
            return JSONResponse(
    ...
    async def _check_rate_limit(self, request: Request) -> None:
        ...
        recent_requests = [req_time for req_time in self.rate_limit_store[client_ip] if req_time > cutoff_time]
    ...
    async def _validate_security_headers(self, request: Request) -> None:
        ...
        suspicious_agents = ["sqlmap", "nmap", "nikto", "burp", "zap", "w3af", "masscan", "nessus", "openvas"]
        ...
        for _header_name, header_value in request.headers.items():
            if any(char in header_value for char in ["\r", "\n", "\0"]):
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid characters in headers")
...
class JSONValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if request.method not in ["GET", "HEAD", "OPTIONS"] and request.headers.get("content-type", "").startswith(
            "application/json"
        ):
            try:
                ...
                if body:
                    try:
                        ...
                        logger.warning(
                            "Invalid JSON in request", extra={"method": request.method, "path": request.url.path}
                        )
                        return JSONResponse(
                ...
                logger.error(
                    "JSON processing error", exc_info=True, extra={"method": request.method, "path": request.url.path}
                )
                return JSONResponse(
                    status_code=status.HTTP_400_BAD_REQUEST, content={"detail": "Request processing error"}
                )
