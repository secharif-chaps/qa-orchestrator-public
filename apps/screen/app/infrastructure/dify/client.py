...
class DifyClient:
    ...
    async def trigger_workflow(
        ...
        callback_payload_template = {"task_id": task_id, "company_id": company_id, "task_type": task_type}
        ...
        payload = {"inputs": inputs, "response_mode": "blocking", "user": f"company_{company_id}"}
        ...
        try:
            ...
            if async_mode:
                ...
                return {
                    "status": "timeout",
                    "task_id": task_id,
                    "message": f"{task_type} request timeout but workflow may be running",
                }
            else:
    ...
    async def send_chat_message(
        self, message: str, company_context: dict[str, Any], chat_history: list = None
    ) -> dict[str, Any]:
        """
    ...
    async def send_global_chat_message(
        self, message: str, contexts: dict[str, Any], system_context: dict[str, Any], chat_history: list = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                try:
                    ...
                    logger.error(f"Error processing Dify chat response: {str(e)}")
                    return {
                        "output": "Je suis désolé, une erreur s'est produite lors du traitement de votre message. Veuillez réessayer.",
                        "status": "error",
                    }
