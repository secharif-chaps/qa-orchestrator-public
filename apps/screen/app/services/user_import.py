...
This module provides functionality to import multiple users from CSV/Excel data
...
async def check_existing_users(
    ...
    Args:
        users: List of user rows to check
...
async def import_users_bulk(
    ...
    logger.info("Checking for existing users in Keycloak")
    ...
    for idx, user in enumerate(request.users):
        ...
        try:
            ...
            # Determine password
            # generate_passwords=true means generate for ALL users (override CSV passwords)
            # generate_passwords=false means use CSV passwords, but still generate for users without one
            if request.generate_passwords:
                # Override all passwords - generate for everyone
                password = keycloak_admin_service._generate_temp_password(length=12)
                result.generated_password = password
            elif user.password:
                # Use password from CSV file
                password = user.password
            else:
                # No password in CSV - must generate one
                password = keycloak_admin_service._generate_temp_password(length=12)
                result.generated_password = password
            ...
            if not org_success:
                ...
                # We still count this as partial success
                logger.warning(
