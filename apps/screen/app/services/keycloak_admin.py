...
class KeycloakAdminService:
            ...
            try:
                ...
                if is_last_attempt:
                    ...
                    logger.error(
                        ...
                        exc_info=True,
                        extra={"max_retries": max_retries, "error_type": type(e).__name__, "error_message": str(e)},
                    )
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                ...
                if is_last_attempt:
                    logger.error(
                        "❌ All retry attempts exhausted for admin token refresh",
                        extra={"status_code": e.response.status_code, "response_text": e.response.text},
                    )
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def _make_admin_request(self, method: str, endpoint: str, data: dict | None = None) -> httpx.Response:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                logger.info(
                    ...
                    extra={"user_id": user_id, "location_header": location, "username": user_data["username"]},
                )
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail=f"Failed to create user: {error_text}"
                )
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal error creating user"
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    ...
                    extra={"user_id": user_id, "status_code": response.status_code, "response_text": response.text},
                )
                return None
            ...
            logger.error(
                ...
                exc_info=True,
                extra={"user_id": user_id, "error_type": type(e).__name__, "error_message": str(e)},
            )
            return None
    ...
    async def search_users(
        ...
        try:
            ...
            logger.info(
                ...
                extra={"search": search, "first": first, "max_results": max_results, "endpoint": endpoint},
            )
            ...
            if response.status_code == 200:
                users = response.json()
                logger.info("Successfully searched users", extra={"result_count": len(users), "search": search})
                return users
            else:
                logger.error(
            ...
            logger.error(
                ...
                exc_info=True,
                extra={"search": search, "error_type": type(e).__name__, "error_message": str(e)},
            )
            return []
    ...
    async def count_users_with_search(self, search: str | None = None) -> int:
        ...
        try:
            ...
            logger.info("Counting users in Keycloak", extra={"search": search, "endpoint": endpoint})
            ...
            if response.status_code == 200:
                ...
                logger.info("Successfully counted users", extra={"count": count, "search": search})
                return count
            else:
                logger.error(
            ...
            logger.error("Failed to parse user count response", extra={"response_text": response.text, "error": str(e)})
            return 0
        except Exception as e:
            logger.error(
    ...
    async def get_user_organization_optimized(self, user_id: str) -> dict[str, Any] | None:
        ...
        """
        try:
            logger.info("Getting organization for user", extra={"user_id": user_id})

            # First, get all organizations
            all_orgs = await self.get_organizations()

            if not all_orgs:
                logger.info("No organizations found", extra={"user_id": user_id})
                return None
            ...
            async def check_membership(org: dict[str, Any]) -> dict[str, Any] | None:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to check membership for org {org_id}", extra={"error": str(e), "org_id": org_id}
                    )

                return None

            # Run all membership checks in parallel
            results = await asyncio.gather(*[check_membership(org) for org in all_orgs], return_exceptions=True)
            ...
            logger.info("User has no organization", extra={"user_id": user_id})
            return None
    ...
    async def send_password_reset_email(self, user_id: str) -> bool:
        ...
        try:
            ...
            response = await self._make_admin_request(
                "PUT", f"/users/{user_id}/execute-actions-email", ["UPDATE_PASSWORD"]
            )
    ...
    async def set_user_password(self, user_id: str, password: str, temporary: bool = True) -> bool:
        ...
        """
        try:
            logger.info("Setting user password directly", extra={"user_id": user_id, "temporary": temporary})
            ...
            if response.status_code == 204:
                logger.info("User password set successfully", extra={"user_id": user_id, "temporary": temporary})
                return True
            elif response.status_code == 404:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
            else:
                logger.error(
                    f"Failed to set user password: {response.status_code} - {response.text}", extra={"user_id": user_id}
                )
                return False
    ...
    async def sync_user_realm_roles(self, user_id: str, target_roles: list[str]) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Successfully synced roles for user {user_id}: +{len(roles_to_add)}, -{len(roles_to_remove)}"
                )
        ...
        try:
            ...
            if response.status_code in [204, 201]:
                ...
                logger.error(
                    ...
                    extra={"organization_id": organization_id, "user_id": user_id, "response_text": response.text},
                )
    ...
    async def search_organizations(self, search: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Searching organizations in Keycloak", extra={"search": search, "endpoint": endpoint})

            response = await self._make_admin_request("GET", endpoint)

            if response.status_code == 200:
                orgs = response.json()
                logger.info("Successfully searched organizations", extra={"result_count": len(orgs), "search": search})
                return orgs
            else:
                logger.error(
    ...
    async def get_organizations(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                orgs = response.json()
                logger.info("Successfully fetched organizations from Keycloak", extra={"organization_count": len(orgs)})
                return orgs
            else:
                logger.error(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    ...
                    extra={"organization_id": organization_id, "organization_name": org.get("name")},
                )
                return org
            elif response.status_code == 404:
                logger.warning("Organization not found in Keycloak", extra={"organization_id": organization_id})
                raise HTTPException(
            ...
            logger.error(
                ...
                exc_info=True,
                extra={"organization_id": organization_id, "error_type": type(e).__name__, "error_message": str(e)},
            )
            return None
    ...
    async def get_user_sessions(self, user_id: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    "Successfully fetched user sessions", extra={"user_id": user_id, "session_count": len(sessions)}
                )
                return sessions
            elif response.status_code == 404:
    ...
    async def revoke_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                ...
                exc_info=True,
                extra={"session_id": session_id, "error_type": type(e).__name__, "error_message": str(e)},
            )
            return False
    ...
    async def get_user_events(
        self, user_id: str, first: int = 0, max_results: int = 20, event_types: list[str] | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            logger.info(
                ...
                extra={"user_id": user_id, "first": first, "max_results": max_results, "event_types": event_types},
            )
            ...
            if response.status_code == 200:
                ...
                logger.info("Successfully fetched user events", extra={"user_id": user_id, "event_count": len(events)})
                return events
            else:
                logger.error(
    ...
    async def get_user_organizations(self, user_id: str) -> list[dict[str, Any]]:
        ...
        """
        try:
            logger.info("Fetching organizations for user from Keycloak", extra={"user_id": user_id})
