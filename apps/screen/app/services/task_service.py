...
cleaning them up on read operations without requiring scheduled tasks.
"""
...
class TaskService:
    ...
    def __init__(self, db: Session):
        self.db = db
        self.timeout_minutes = getattr(settings, "TASK_TIMEOUT_MINUTES", DEFAULT_TASK_TIMEOUT_MINUTES)
    ...
    def cleanup_stale_tasks_for_company(self, company_id: int) -> int:
        ...
        cleaned_count = 0
        error_message = f"Task timeout - stuck in running state for more than {self.timeout_minutes} minutes"
    ...
    def get_stale_tasks(self, limit: int | None = None) -> list[Task]:
