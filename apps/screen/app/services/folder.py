...
class FolderService:
    ...
    @staticmethod
    def create_folder(
        db: Session, organization_id: str, owner_id: str, owner_username: str, folder_data: FolderCreate
    ) -> Folder:
        """Create a new folder.
    ...
    @staticmethod
    def get_folder(
        db: Session, folder_id: UUID, organization_id: str, include_deleted: bool = False
    ) -> Folder | None:
        """Get a folder by ID.
        ...
        """
        query = db.query(Folder).filter(Folder.id == folder_id, Folder.organization_id == organization_id)
    ...
    def _get_folder_items_summary(
        db: Session, folder_id: UUID, item_archived_filter: bool = False
    ) -> list[dict[str, Any]]:
    ...
    def get_folder_with_items(
        db: Session, folder_id: UUID, organization_id: str, item_archived_filter: bool = False
    ) -> dict[str, Any] | None:
        ...
        items = FolderService._get_folder_items_summary(db, folder_id, item_archived_filter=item_archived_filter)
    ...
    def list_all_org_folders(
        ...
        for folder in folders:
            logger.debug(
                f"  - Folder: {folder.id} | {folder.name} | owner: {folder.owner} | deleted: {folder.is_deleted}"
            )
    ...
    @staticmethod
    def remove_item_from_folder(db: Session, folder_id: UUID, item_id: str, item_type: str) -> bool:
        """Remove an item from a folder."""
        folder_item = (
            db.query(FolderItem)
            .filter(FolderItem.folder_id == folder_id, FolderItem.item_id == item_id, FolderItem.item_type == item_type)
            .first()
        )
    ...
    def update_item_folder(
        db: Session, folder_id: UUID, item_id: str, item_type: str, destination_folder_id: UUID, organization_id: str
    ) -> FolderItem | None:
        ...
        folder_item.added_at = datetime.now(UTC)  # Update timestamp to reflect move
        db.commit()
        db.refresh(folder_item)
    ...
    @staticmethod
    def get_folders_for_item(db: Session, item_id: str, item_type: str, organization_id: str) -> list[Folder]:
        """Get all folders containing a specific item."""
        folder_items = (
            db.query(FolderItem).filter(FolderItem.item_id == item_id, FolderItem.item_type == item_type).all()
        )
        ...
        return (
            db.query(Folder)
            .filter(Folder.id.in_(folder_ids), Folder.organization_id == organization_id, not Folder.is_deleted)
            .all()
        )
    ...
    @staticmethod
    def share_folder(db: Session, folder_id: UUID, user_id: str, user_username: str, role: ShareRole) -> FolderShare:
        ...
        existing = (
            db.query(FolderShare).filter(FolderShare.folder_id == folder_id, FolderShare.user_id == user_id).first()
        )
    ...
    @staticmethod
    def unshare_folder(db: Session, folder_id: UUID, user_id: str) -> bool:
        ...
        share = db.query(FolderShare).filter(FolderShare.folder_id == folder_id, FolderShare.user_id == user_id).first()
    ...
    @staticmethod
    def get_folder_shares(db: Session, folder_id: UUID) -> list[FolderShare]:
    ...
    @staticmethod
    def update_share_role(db: Session, folder_id: UUID, user_id: str, role: ShareRole) -> FolderShare | None:
    ...
    def has_folder_access(
        ...
        folder = db.query(Folder).filter(Folder.id == folder_id, Folder.organization_id == organization_id).first()
    ...
    @staticmethod
    def get_user_folder_role(db: Session, folder_id: UUID, user_id: str) -> str | None:
    ...
    @staticmethod
    def is_folder_owner(db: Session, folder_id: UUID, user_id: str) -> bool:
    ...
    @staticmethod
    def flag_folder_orphaned(db: Session, folder_id: UUID) -> bool:
    ...
    @staticmethod
    def add_favorite(db: Session, folder_id: UUID, user_id: str) -> bool:
        ...
        existing = (
            db.query(UserFolderFavorite)
            .filter(UserFolderFavorite.folder_id == folder_id, UserFolderFavorite.user_id == user_id)
            .first()
        )
    ...
    @staticmethod
    def remove_favorite(db: Session, folder_id: UUID, user_id: str) -> bool:
    ...
    @staticmethod
    def is_favorite(db: Session, folder_id: UUID, user_id: str) -> bool:
    ...
    @staticmethod
    def get_user_favorite_folder_ids(db: Session, user_id: str, organization_id: str) -> set[UUID]:
        ...
        """
        favorites = (
            db.query(UserFolderFavorite.folder_id)
            .join(Folder, UserFolderFavorite.folder_id == Folder.id)
            .filter(
                UserFolderFavorite.user_id == user_id, Folder.organization_id == organization_id, not Folder.is_deleted
            )
            .all()
        )
    ...
    def user_has_company_access(
        ...
        if user_roles and _user_is_manager(user_roles):
            ...
            company = (
                db.query(Company).filter(Company.id == company_id, Company.organization_id == organization_id).first()
            )
            if company:
    ...
    def get_accessible_company_ids(
        db: Session, user_id: str, organization_id: str, username: str | None = None
    ) -> set[int]:
        ...
        accessible_folders = FolderService.list_folders(
            db=db, organization_id=organization_id, user_id=user_id, archived=False, username=username
        )
        ...
        folder_items = (
            db.query(FolderItem.item_id)
            .filter(FolderItem.folder_id.in_(folder_ids), FolderItem.item_type == "company")
            .all()
        )
        ...
        logger.debug(f"User {user_id} has access to {len(company_ids)} companies via {len(accessible_folders)} folders")
