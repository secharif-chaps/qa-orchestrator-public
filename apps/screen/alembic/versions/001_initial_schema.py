...
def upgrade():
    ...
    op.execute(
        "CREATE TYPE task_type_enum AS ENUM ('profile', 'digital', 'timeline', 'products', 'jobs', 'csr', 'press', 'team', 'data_collection')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM(
                "pending", "blocked", "running", "succeeded", "error", name="task_status_enum", create_type=False
            ),
            nullable=False,
        ),
        sa.Column("error", sa.String(), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column(
            "module_name",
            postgresql.ENUM("screen", "target", "explore", name="modulename", create_type=False),
            nullable=False,
        ),
    ...
    op.create_index(
        "idx_user_preferences_jsonb", "user_preferences", ["preferences"], unique=False, postgresql_using="gin"
    )
    op.create_index("idx_user_preferences_updated_at", "user_preferences", ["updated_at"], unique=False)
    ...
    op.execute(
        "CREATE INDEX idx_organization_cost_summary_organization_id ON organization_cost_summary (organization_id)"
    )
