"""Tests for company data cleanup verification.

These tests verify that:
1. Company CRUD works without JSON columns
2. Agent callback writes only to normalized tables
...
class TestAgentCallbackWritesToNewTables:
    """Test that agent callback writes only to normalized tables."""
    ...
    def test_profile_data_written_to_normalized_table(self, mock_db_session):
        ...
        # Write the data
        save_profile_data(mock_db_session, company_id, agent_data["profile"])
    ...
    def test_write_section_data_dispatches_correctly(self, mock_db_session):
        ...
        section_types = ["profile", "digital", "timeline", "products", "jobs", "csr", "press", "team"]
...
class TestNoReferencesToOldJsonFields:
    ...
    def test_read_all_section_data_returns_dict(self, mock_db_session):
        ...
        expected_keys = ["profile", "digital", "timeline", "products", "jobs", "csr", "press", "team"]
