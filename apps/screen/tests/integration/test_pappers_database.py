...
class TestOrganizationFeatureFlagConfig:
    ...
    def test_config_can_store_api_key(self, db_session):
        ...
        # Retrieve and verify
        saved_flag = (
            db_session.query(OrganizationFeatureFlag)
            .filter_by(organization_id="test-org-123", flag=FeatureFlag.PAPPERS)
            .first()
        )
    ...
    def test_enable_disable_preserves_api_key(self, db_session):
        ...
        feature_flag = OrganizationFeatureFlag(
            organization_id="test-org-456", flag=FeatureFlag.PAPPERS, enabled=True, config={"api_key": "my-secret-key"}
        )
        db_session.add(feature_flag)
        db_session.commit()
