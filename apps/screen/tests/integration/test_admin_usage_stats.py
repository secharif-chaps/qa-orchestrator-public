...
class TestCompaniesCountQuery:
    ...
    def test_companies_count_filters_by_date_range(self, db_session):
        ...
        db_session.execute(Company.__table__.update().where(Company.id == company1.id).values(created_at=yesterday))
        ...
        db_session.flush()
        db_session.execute(Company.__table__.update().where(Company.id == company2.id).values(created_at=last_week))
