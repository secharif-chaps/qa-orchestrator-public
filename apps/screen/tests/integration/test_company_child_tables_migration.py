...
class TestIndexesCreatedOnCompanyIdColumns:
    ...
    def test_company_id_index_on_correct_column(self, db_engine):
        ...
        for table_name in CHILD_TABLES:
            indexes = inspector.get_indexes(table_name, schema=SCHEMA)
            company_id_indexes = [idx for idx in indexes if "company_id" in idx["name"]]

            assert len(company_id_indexes) >= 1, f"No company_id index found on {table_name}"
...
class TestEnumTypesCreatedCorrectly:
    ...
    def test_product_item_type_column_uses_enum(self, db_session):
        ...
        assert result[0] == "product_item_type_enum", f"Expected product_item_type_enum, got {result[0]}"
    ...
    def test_csr_initiative_type_column_uses_enum(self, db_session):
        ...
        assert result is not None, "type column not found"
        assert result[0] == "csr_initiative_type_enum", f"Expected csr_initiative_type_enum, got {result[0]}"
    ...
    def test_press_item_type_column_uses_enum(self, db_session):
        ...
        assert result is not None, "type column not found"
        assert result[0] == "press_item_type_enum", f"Expected press_item_type_enum, got {result[0]}"
...
class TestSelfReferentialFKForTeamMembers:
    ...
    def test_parent_id_fk_references_same_table(self, db_engine):
        ...
        parent_fk = next((fk for fk in fks if "parent_id" in fk["constrained_columns"]), None)
        ...
        assert parent_fk["referred_columns"] == ["id"], "parent_id FK should reference id column"
    ...
    def test_parent_id_is_nullable(self, db_engine):
        ...
        parent_id_col = next((col for col in columns if col["name"] == "parent_id"), None)
    ...
    def test_parent_id_index_exists(self, db_engine):
        ...
        assert "idx_team_members_parent_id" in index_names, "Index on parent_id not found"
...
class TestOnDeleteBehaviors:
    ...
    def test_company_cascade_deletes_child_records(self, db_session):
        ...
        try:
            ...
            for table_name in CHILD_TABLES:
                result = db_session.execute(
                    text(f"SELECT COUNT(*) FROM {SCHEMA}.{table_name} WHERE company_id = :id"), {"id": test_company_id}
                ).scalar()
    ...
    def test_parent_set_null_on_delete(self, db_session):
        ...
        try:
            ...
            db_session.execute(text(f"DELETE FROM {SCHEMA}.company_team_members WHERE id = 91101"))
            db_session.commit()
...
class TestTableSchemaCorrectness:
    ...
    def test_online_services_has_correct_columns(self, db_engine):
        ...
        expected = {"id", "company_id", "name", "name_source", "description", "description_source", "created_at"}
    ...
    def test_product_categories_has_array_columns(self, db_engine):
        ...
        assert "ARRAY" in str(items_col["type"]).upper(), f"items should be ARRAY type, got {items_col['type']}"
