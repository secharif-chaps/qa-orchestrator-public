...
class TestEnumFieldValidation:
    ...
    def test_create_csr_initiative_with_all_types(self, db_session, sample_company):
        ...
        for init_type, value in types_to_test:
            initiative = CompanyCsrInitiative(
                company_id=sample_company.id, type=init_type, value=value, value_source="Chaps-e"
            )
            db_session.add(initiative)
        ...
        initiatives = (
            db_session.query(CompanyCsrInitiative).filter(CompanyCsrInitiative.company_id == sample_company.id).all()
        )
    ...
    def test_create_press_item_with_all_types(self, db_session, sample_company):
        ...
        for press_type, value in types_to_test:
            press = CompanyPressItem(
                company_id=sample_company.id, type=press_type, value=value, value_source="https://news.com"
            )
            db_session.add(press)
        ...
        # Verify all were created
        items = db_session.query(CompanyPressItem).filter(CompanyPressItem.company_id == sample_company.id).all()
        assert len(items) == 7
...
class TestCascadeDeleteFromCompany:
    ...
    def test_cascade_delete_removes_online_services(self, db_session, sample_company):
        ...
        services = [
            CompanyOnlineService(company_id=sample_company.id, name=f"Service {i}", name_source="https://example.com")
            for i in range(3)
        ]
        ...
        count_before = (
            db_session.query(CompanyOnlineService).filter(CompanyOnlineService.company_id == sample_company.id).count()
        )
    ...
    def test_cascade_delete_removes_all_child_types(self, db_session, sample_company):
        ...
        db_session.add(
            CompanyOnlineService(company_id=sample_company.id, name="Service", name_source="https://example.com")
        )
        db_session.add(
            CompanySocialMediaAccount(
                company_id=sample_company.id, platform="Instagram", platform_source="https://example.com"
            )
        )
        db_session.add(
            CompanyTimelineEvent(company_id=sample_company.id, title="Event", title_source="https://example.com")
        )
        db_session.add(
            CompanyProductItem(
                company_id=sample_company.id,
        ...
        db_session.add(
            CompanyProductCategory(company_id=sample_company.id, category_name="Category", items=["Item 1", "Item 2"])
        )
        db_session.add(CompanyJobOffer(company_id=sample_company.id, title="Job", title_source="https://example.com"))
        db_session.add(
            CompanyCsrInitiative(
                company_id=sample_company.id,
                type=CsrInitiativeType.sustainability,
...
class TestTeamMemberHierarchy:
    ...
    def test_delete_parent_sets_null_for_subordinates(self, db_session, sample_company):
        ...
        db_session.execute(CompanyTeamMember.__table__.delete().where(CompanyTeamMember.id == ceo_id))
        db_session.commit()

        # Refresh director and verify parent_id is now NULL
        director_after = db_session.query(CompanyTeamMember).filter(CompanyTeamMember.id == director_id).first()
...
class TestProductCategoryWithArrays:
    ...
    def test_create_product_category_with_items_array(self, db_session, sample_company):
        ...
        category = CompanyProductCategory(
            company_id=sample_company.id, category_name="Fashion", items=["Clothing", "Accessories", "Footwear"]
        )
    ...
    def test_create_product_category_with_translations(self, db_session, sample_company):
        ...
        category = CompanyProductCategory(
            company_id=sample_company.id, category_name="Electronics", items=["Phones", "Laptops", "Tablets"]
        )
        db_session.add(category)
        db_session.commit()
        db_session.refresh(category)
...
class TestRelationshipsFromCompany:
    ...
    def test_company_to_child_relationships(self, db_session, sample_company):
        ...
        db_session.add(
            CompanyOnlineService(company_id=sample_company.id, name="Service 1", name_source="https://example.com")
        )
        db_session.add(
            CompanySocialMediaAccount(
                company_id=sample_company.id, platform="Twitter", platform_source="https://example.com"
            )
        )
        db_session.add(
            CompanyTimelineEvent(company_id=sample_company.id, title="Event 1", title_source="https://example.com")
        )
        db_session.add(
            CompanyProductItem(
                company_id=sample_company.id,
        ...
        db_session.add(
            CompanyProductCategory(company_id=sample_company.id, category_name="Category 1", items=["Item 1"])
        )
        db_session.add(CompanyJobOffer(company_id=sample_company.id, title="Job 1", title_source="https://example.com"))
        db_session.add(
            CompanyCsrInitiative(
                company_id=sample_company.id,
