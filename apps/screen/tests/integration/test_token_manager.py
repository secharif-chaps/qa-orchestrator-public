...
class TestAddTokens:
    ...
    def test_add_tokens_creates_organization_if_not_exists(self, db_session):
        ...
        transaction = db_session.query(TokenTransaction).filter_by(organization_id=org_id).first()
        assert transaction is not None
        assert transaction.amount == tokens_to_add
...
class TestConsumeTokens:
    ...
    def test_consume_tokens_with_insufficient_balance(self, db_session):
        ...
        transaction_count = db_session.query(TokenTransaction).filter_by(organization_id=org_id).count()
...
class TestGetTransactionHistory:
    ...
    def test_get_transaction_history_with_type_filter(self, db_session):
        ...
        # Execute with filter
        token_manager = TokenManager(db_session)
        history = token_manager.get_transaction_history(org_id=org_id, transaction_type=TransactionType.consume)
    ...
    def test_get_transaction_history_with_reference_type_filter(self, db_session):
        ...
        # Execute with filter
        token_manager = TokenManager(db_session)
        history = token_manager.get_transaction_history(org_id=org_id, reference_type=ReferenceType.company)
