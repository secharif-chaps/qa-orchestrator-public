...
Conventions Tailwind CSS v4.1 du projet ChapsMind. Ce document est la référence
développeur : tokens sémantiques, mappings Figma, classes dépréciées, breakpoints
custom.

> Pour l'automatisation IA, la skill `tailwind-styling` (`.claude/skills/tailwind-styling/`)
> reprend ces règles dans un format optimisé pour l'assistant.
...
When translating a Figma mockup to code, follow this mapping. Figma tokens use the format
`{usage}-{semantic}-{variant}` where variant can be `base`, `hovered`, `pressed`, `light`,
`lighter`, `muted`, etc.
