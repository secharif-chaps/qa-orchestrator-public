...
class DifyClient:
    ...
    async def trigger_timeline_workflow(
        ...
        """
        Trigger the Dify timeline workflow with callback URLs
        ...
        try:
            if async_mode:
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    # Use the proper Dify API endpoint format: /workflows/{workflow_id}/run
                    response = await client.post(
                        f"{self.base_url}/workflows/{self.timeline_workflow_id}/run",
                        json=payload,
                        headers=headers,
                        timeout=10.0  # Just wait for acknowledgment, not completion
                    )
                    ...
                    if response.status_code not in [200, 201, 202]:
                        error_msg = f"Dify timeline workflow trigger failed with status: {response.status_code}"
                        logger.error(f"{error_msg}. Response: {response.text}")
                        raise Exception(error_msg)
                    
                    logger.info(f"✅ Dify timeline workflow triggered successfully for task {task_id} (async mode)")
                    return {"status": "triggered", "task_id": task_id, "message": "Timeline workflow started, will callback when complete"}
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code != 200:
                        error_msg = f"Dify timeline workflow returned non-200 status code: {response.status_code}"
                        logger.error(f"{error_msg}. Response: {response.text}")
                        raise Exception(error_msg)
                    
                    response_data = response.json()
                    logger.info(f"Dify timeline workflow response received for {company_name} (sync mode)")
                    ...
                    if "data" in response_data and "outputs" in response_data["data"]:
                        ...
                        if "result" in outputs and isinstance(outputs["result"], str):
                            ...
                            try:
                                parsed_result = json.loads(outputs["result"])
                                logger.info(f"Successfully parsed Dify timeline data for {company_name}")
                                return {"timeline": parsed_result}
                            except json.JSONDecodeError:
                                logger.warning(f"Could not parse timeline result as JSON, returning as-is")
                                return {"timeline": outputs}
                        else:
                            return {"timeline": outputs}
                    else:
                        logger.warning(f"Unexpected Dify timeline response structure: {response_data}")
                        return {"timeline": response_data}
                    
        except httpx.TimeoutException as e:
            if async_mode:
                # In async mode, timeout might mean Dify accepted but is still processing
                logger.warning(f"Dify timeline request timed out, but workflow may still be running (will callback)")
                return {"status": "timeout", "task_id": task_id, "message": "Timeline request timeout but workflow may be running"}
            else:
                error_msg = f"Dify timeline workflow request timed out"
                logger.error(error_msg)
                raise Exception(error_msg) from e
        except httpx.HTTPError as e:
            error_msg = f"HTTP error occurred with Dify timeline workflow: {str(e)}"
            logger.error(error_msg)
            raise Exception(error_msg) from e
        except Exception as e:
            error_msg = f"Error triggering Dify timeline workflow: {str(e)}"
            logger.error(error_msg)
            raise Exception(error_msg) from e