...
import {
  ...
  Avatar,           // User representation with initials ONLY
  Badge,            // Icon display with color (prefer variant="secondary")
...
|-----------|---------|
| `Avatar` | User representation with initials ONLY |
| `Badge` | Icon display (prefer `variant="secondary"`) |
| `Bullet` | Simple status dot |
...
| `Select` | Dropdown |
| `Switch` | On/off toggle (use `@update:model-value` NOT `@change`) |
| `Tab` | Content panels |
| `Tag` | Static labels |
...
## Avatar vs Badge (CRITICAL)

These components have **distinct purposes** - do not confuse them:

### Avatar - User Representation Only

Use Avatar **exclusively** for displaying user/person representation with initials or profile images.
...
Use Badge for displaying icons (entity types, categories, actions). **Always prefer `variant="secondary"`** for a lighter, more subtle UI.
