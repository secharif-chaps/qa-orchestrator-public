---
name: symfony-cache
description: Implement caching with Symfony Cache component for PHP/Symfony applications. Use when adding caching to services, configuring cache pools (Redis, filesystem, APCu), implementing cache tag invalidation strategies, optimizing API response times, or warming caches on deployment. Activates when working on performance optimization, cache configuration in config/packages/cache.yaml, or any PHP service that needs to store and retrieve cached data efficiently.
...
- When adding caching to a PHP service or repository
- When configuring cache pools in `config/packages/cache.yaml`
- When implementing cache tag invalidation for related entities
- When optimizing API response times with HTTP caching
- When setting up Redis, filesystem, or APCu cache adapters
- When creating cache warmers for deployment
- When working with `CacheInterface` or `TagAwareCacheInterface`
- When adding `#[Cache]` attributes to controllers
