...
const getValue = <T,>(sourcedValue: SourcedValue<T> | undefined): T | undefined => {
  return sourcedValue?.value
}

// Helper function to get sources from an array of SourcedValue items
const getSourcesFromArray = <T,>(items: SourcedValue<T>[] | undefined): string[] => {
  if (!items || items.length === 0) return []
  return items.map((item) => item.source).filter((source) => source && source.trim() !== '')
}
...
const getSourcesFromObject = (obj: Record<string, any> | undefined): string[] => {
  ...
  Object.values(obj).forEach((value) => {
    if (value && typeof value === 'object') {
      if ('source' in value && typeof value.source === 'string' && value.source.trim() !== '') {
...
const addListItemsImproved = (
  ...
  const bulletText = items.map((item) => `• ${getValue(item)}`).join('\n')
...
const createProductsSlide2 = (pptx, company: Company) => {
  ...
  addListItemsImproved(slide, 'Partner Brands', products.partnerBrands, 1.0, 1.5, 4.0)

  // COLUMN 2: Private Labels
  addListItemsImproved(slide, 'Private Labels', products.privateLabels, 5.0, 1.5, 4.0)
...
const createTitleSlide = (pptx, company: Company) => {
  ...
  slide.addText(
    'This company screening is generated from the screening module\nof the MINT software by ChapsVision',
    {
      x: 0.5,
      y: 4.2,
      w: 9.0,
...
const downloadPPT = async (selectedOptions: string[] = []) => {
  ...
  try {
    ...
    if (
      selectedOptions.includes('targetAudience') &&
      company.value.products &&
      (company.value.products.customerType || company.value.products.marketingPositioning)
    ) {
    ...
    if (
      ...
      company.value.press &&
      company.value.press.articles &&
      company.value.press.articles.length > 0
    ) {
