...
export function useConversationTimeout() {
  ...
  const updateElapsedTime = () => {
    if (!conversationStore.isWaitingForAI || !conversationStore.waitingStartTime) {
      elapsedSeconds.value = 0
      return
    }
    elapsedSeconds.value = Math.floor((Date.now() - conversationStore.waitingStartTime) / 1000)
  }
