...
export function useMercure() {
  ...
  async function getMercureToken(
    ...
    try {
      ...
      const data: MercureTokenResponse = await fetch(
        '/security/real-time/token',
      );
    ...
    const handleError = async (error: Error) => {
      ...
      if (!navigator.onLine) {
        if (newRetryCount >= logThreshold) {
          console.warn(
            `Mercure: waiting for network to come back online (${subscribeKey})`,
          );
        }
        await waitForOnline();
  ...
  async function unsubscribe(subscribeKey: string): Promise<void> {
    const subscription = activeSubscriptions.value.find(
      (sub) => sub.subscribeKey === subscribeKey,
    );
    if (!subscription) {
