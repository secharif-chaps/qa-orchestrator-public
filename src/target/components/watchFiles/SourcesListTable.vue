    ...
    >
      ...
      <div class="modal-select-container relative h-10 w-56">
        <Select
          ...
          :disabled="
            isLoadingSourceTypes || noSourcesToDisplay || !typeOptions.length
          "
          multiple
          class="h-full"
        >
...
const noSourcesToDisplay = computed(
  () => !isLoading.value && !allSources.value.length,
);
...
const isLoading = computed(
  () => isLoadingSources.value && !allSources.value.length,
);
...
const headerColumns = computed(() =>
  columns.value.filter((c) => c.key !== 'checkbox'),
);
...
watch(
  ...
  () => {
    ...
    const allSelected = sources.value.every((source) =>
      selectedSources.value.includes(source.id),
    );
    const someSelected = sources.value.some((source) =>
      selectedSources.value.includes(source.id),
    );
