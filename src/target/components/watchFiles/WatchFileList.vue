        ...
        <Table :loading="isLoading && status !== 'success'" :items="watchFiles" :fields="columns">
          ...
          <template #cell(name)="{ value, item }">
            <td class="of-flex of-ml-2 px-4 py-3">
              <RouterLink :to="{ name: RouteNames.WATCH_FILES, params: { id: item.id } }" class="text-primary-600 hover:underline">
                {{ value }}
              </RouterLink>
            </td>
          </template>
          ...
          <template #cell(newContent)="{ item }">
            <td class="px-4 py-3">
              <RouterLink
                ...
                :to="{ name: RouteNames.WATCH_FILES, params: { id: item.id }, query: { tab: 'news' } }"
