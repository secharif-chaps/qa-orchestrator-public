<template>
  <div class="flex h-full flex-col">
    ...
    />
    <SourcesListEmpty
      v-else-if="!isLoading && !hasSources"
...
const allSources = computed(() => {
  if (!sourcesData.value?.groups || !Array.isArray(sourcesData.value.groups))
    return [];

  // Flatten all sources from all groups
  return sourcesData.value.groups.reduce(
    (acc: Source[], group: SourceGroup) => {
