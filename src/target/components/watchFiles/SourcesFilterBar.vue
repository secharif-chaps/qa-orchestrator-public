    ...
    <div class="flex items-center justify-between">
      ...
      <div v-if="showTypeFilter" class="relative h-10 w-56">
        <Select
          ...
          :placeholder="
            $t('watch_files.actors.detail.sources.type_filter_placeholder')
          "
    ...
    filtered = filtered.filter(
      (source) =>
        ...
        (source.primaryDomain &&
          source.primaryDomain.toLowerCase().includes(searchTerm)),
    );
  }

  if (selectedTypes.value.length > 0) {
    filtered = filtered.filter(
      (source) => source.type && selectedTypes.value.includes(source.type),
    );
  }
