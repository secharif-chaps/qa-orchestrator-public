<template>
  <div class="flex flex-col gap-3">
    ...
    >
      {{
        t(
          `watch_files.filters.type.event_types.see.${displayAllEventTypes ? 'less' : 'more'}`,
        )
      }}
    </Button>
    <Button
...
const filteredEventTypes = computed(() =>
  eventTypes.filter(({ type: eventType }) => {
    ...
    return (
      eventType
        .toLocaleLowerCase()
        .includes(searchEventType.value.toLocaleLowerCase()) || isSelected
    );
  }),
);
...
const displayedEventTypes = computed(() =>
  displayAllEventTypes.value
    ? filteredEventTypes.value
    : slicedEventTypes.value,
);
