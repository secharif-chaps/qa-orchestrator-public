...
import {
    getWatchFileEvents,
    getWatchFileEventsLink,
} from '~/api/watchFileEvents';
...
export const WATCH_FILE_EVENTS_QUERY_KEYS = {
  ...
  byLink: (link: string) =>
    [...WATCH_FILE_EVENTS_QUERY_KEYS.root, link] as const,
...
export const getWatchFileEventsQueryLink = defineQueryOptions(
  ({ link }: { link: string }) => ({
...
export const useWatchFileEventsInfiniteQuery = (
  watchFileId: ComputedRef<string>,
) => {
...
export const getWatchFileEventsQuery = defineQueryOptions(
  ({
    ...
    key: WATCH_FILE_EVENTS_QUERY_KEYS.byWatchFile(
      watchFileId,
      sortBy,
      sortOrder,
    ),
