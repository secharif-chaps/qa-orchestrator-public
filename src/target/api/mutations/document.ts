  ...
  const { mutate, ...mutation } = useMutation({
    ...
    onMutate: ({ documentId, watchFileId }) => {
      const documentsCollection = queryCache.getQueryData<{
        ...
        DOCUMENT_QUERY_KEYS.withFilters(
          watchFileId,
          watchFileDocumentsStore.queryParams,
        ),
      );

      const oldCollection = JSON.parse(JSON.stringify(documentsCollection));

      const findDocument = documentsCollection?.items.find(
      ...
      queryCache.cancelQueries({
        key: DOCUMENT_QUERY_KEYS.withFilters(
          watchFileId,
          watchFileDocumentsStore.queryParams,
        ),
      });
...
export const useDocumentValidation = (
  options?: CallbackMutations<DocumentValidationResponse>,
) => {
  ...
  const { mutate, ...mutation } = useMutation({
    ...
    onMutate: ({ watchFileId, documentId, action }) => {
      ...
      const oldCollection = JSON.parse(
        JSON.stringify(documentsCollection ?? null),
      );
      ...
      if (findDocument) {
        findDocument.manualStatus =
          action === DocumentValidationAction.UNCERTAIN ? null : action;
      }
    ...
    onError: (
      error,
      { watchFileId },
      { oldCollection, documentsCollection },
    ) => {
      if (
        documentsCollection ===
    ...
    onSuccess(data, { action, watchFileId, documentId }) {
      ...
      const itemDocument = queryCache.getQueryData<Document>(
        DOCUMENT_QUERY_KEYS.byId(documentId),
      );
      ...
      if (
        ...
        ((data.validated_at && data.validated_by) ||
          action === DocumentValidationAction.UNCERTAIN)
      ) {
  ...
  const { mutate, ...mutation } = useMutation({
    ...
    onSuccess(data, { watchFileId, action }, { oldCollection }) {
      ...
      if (data.failed_count > 0) {
        toast.warning(
          t(
            'watch_files.documents.batch_status_change.partial_success_' +
              action,
            {
