...
export const useWatchFileFiltersStore = defineStore('watchFileFilters', () => {
  ...
  const datesFilterCount = (type: FilterType) => {
    ...
    if (type === 'documents') {
      ...
      if (docState.selectedDateType) {
        if (
          docState.datesPicker &&
          (docState.datesPicker.start || docState.datesPicker.end)
        ) {
          return 1;
        }
      ...
      if (
        state.datesPicker &&
        (state.datesPicker.start || state.datesPicker.end)
      ) {
        return 1;
      }
  ...
  const filtersCounts = (type: FilterType) => {
    ...
    if (type === 'documents') {
      const docState = state as DocumentsFilterState;
      return (
        docState.status.length +
        datesCount +
        docState.actors.length +
        docState.sources.length
      );
  ...
  const removeActor = (type: FilterType, id: string) => {
    const state = getState(type);
    const itemIndex = state.formFilters.actors.findIndex(
      (item) => item.id === id,
    );
  ...
  const removeSource = (type: FilterType, id: string) => {
    const state = getState(type);
    const itemIndex = state.formFilters.sources.findIndex(
      (item) => item.id === id,
    );
  ...
  const removeEventType = (type: FilterType, eventType: WatchFileEventType) => {
    ...
    const itemIndex = state.formFilters.eventTypes.findIndex(
      (item) => item === eventType,
    );
    if (itemIndex !== -1) {
  ...
  const removeValidation = (type: FilterType, id: string) => {
    ...
    const statusIndex = state.formFilters.status.findIndex(
      (status) => status === id,
    );
  ...
  const initializeFromUrl = (
    ...
    if (query.actors && facets) {
      const actorIds = Array.isArray(query.actors)
        ? query.actors
        : [query.actors];
      ...
      if (type === 'documents' && 'actors' in facets) {
        ...
        if (docFacets.actors) {
          actorsFromFacets = actorIds
            .map((id) => {
              const facet = docFacets.actors.find(
                (f) => f.actor.id === String(id),
              );
              return facet?.actor;
            })
    ...
    if (query.sources && facets && 'sources' in facets) {
      const sourceIds = Array.isArray(query.sources)
        ? query.sources
        : [query.sources];
    ...
    if (type === 'documents') {
      ...
      if (query.status) {
        const statusArray = Array.isArray(query.status)
          ? query.status
          : [query.status];
        docState.formFilters.status = statusArray.filter(
      ...
      if (
        query.selectedDateType &&
        typeof query.selectedDateType === 'string'
      ) {
        docState.formFilters.selectedDateType =
          query.selectedDateType as FilterDates;
        docState.selectedDateType = docState.formFilters.selectedDateType;
      }
    }
      ...
      if (query.eventTypes) {
        ...
        const validEventTypes = eventTypesArray.filter(
          (et): et is WatchFileEventType =>
            typeof et === 'string' &&
            ['created', 'updated', 'deleted', 'validated', 'rejected'].includes(
              et,
            ),
        );
    ...
    if (query.datesPickerStart || query.datesPickerEnd) {
      const startDate = query.datesPickerStart
        ...
        : undefined;
      const endDate = query.datesPickerEnd
        ? new Date(query.datesPickerEnd as string)
        : undefined;
