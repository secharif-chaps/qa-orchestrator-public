...
export function parseCSV(content: string): CSVParseResult {
  ...
  if (nameIndex === -1) {
    errors.push(`Could not find company name column. Expected one of: ${NAME_VARIATIONS.slice(0, 5).join(', ')}, etc.`)
  }
  
  if (websiteIndex === -1) {
    errors.push(`Could not find website column. Expected one of: ${WEBSITE_VARIATIONS.slice(0, 5).join(', ')}, etc.`)
  }
